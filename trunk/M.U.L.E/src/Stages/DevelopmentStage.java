...
/**
 * DevelopmentStage updates the View and the Model during the Development part
 * of the game. During Development, users are allowed to interact with the town
 * and the map by visiting town stores and placing mules on land plots.
 * 
 * @author jabad3
 * 
 */
public class DevelopmentStage extends Stage implements MuleTimerListener,
     ...
     * 
     * @param mainFrame
     *            The JFrame to hold the View
     * @param gameModel
     *            The game model used throughout the game
     */
	...
	myView = new DevelopmentView(mapPanel, townPanel, new PlayerPawn(
		new ImageIcon("buzzite.png")), muleTimerPanel); // temporary
    ...
    /**
     * Begins the current player's turn by... 1) determining the current player
     * 2) updating the view to display the correct pawn 3) reseting the view to
     * put the player in the correct spot with correct amount of time remaining
     */
    private void beginCurrentPlayerTurn() {
	currentPlayer = playerList.get(currentPlayerIndex);
	randService.startService(currentPlayer, gameModel.getCurrentRound(),
		gameModel.isInLastPlace(currentPlayer), mainFrame);
    ...
    /**
     * Advance the stage one turn. Either start the next player's turn, or, if
     * there is no next player, start the next stage.
     */
	...
	StorePanel storepanel = new StorePanel(gameModel.getStore(),
		currentPlayer, new ActionListener() {
     ...
     * Pub payment depends on the player's time remaining as well as the current
     * round of the game.
     * 
	...
	if (gameModel.getCurrentRound() < 4)
	    roundBonus = 50;
	else if (gameModel.getCurrentRound() < 8
		&& gameModel.getCurrentRound() >= 4)
	    roundBonus = 100;
	else if (gameModel.getCurrentRound() < 12
		&& gameModel.getCurrentRound() >= 8)
	    roundBonus = 150;
	else if (gameModel.getCurrentRound() == 12)
	    roundBonus = 200;
	else
     ...
     * 
     * @param player
     *            The player that the land plot gives its mule to
     * @param plot
     *            The land plot that the player gives its mule to
     */
    ...
    /**
     * Causes the current player's mule to run away by setting its mule to null.
     */
