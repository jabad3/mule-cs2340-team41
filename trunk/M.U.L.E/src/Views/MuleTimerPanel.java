...
/**
 * This class displays a timer in the Mule game. A timer acts as a visual cue to
 * the user to let them know how much time is left in their current turn.
 * 
 * The MuleTimerPanel will not automatically update itself. The decrement method
 * must be called whenever the timer needs to be adjusted.
 * 
 * A java.util.Timer or javax.swing.Timer should be set up to perform an action
 * every X number of milliseconds. A method call to decrement can then be
 * included with the other actions that are invoked by the java.util.Timer or
 * javax.swing.Timer to get the MuleTimerPanel to update accordingly.
 * 
...
public class MuleTimerPanel extends JPanel {
    ...
    /**
     * Creates a MuleTimerPanel with default duration of 30 seconds and default
     * color of red.
     */
    ...
    /**
     * Creates a MuleTimerPanel with a specified duration and default bar color
     * of red.
     * 
     * @param duration
     *            Duration of the timer in milliseconds
     ...
     * 
     * @param listener
     *            A new MuleTimerListener to send notifications to
     */
     ...
     * the default value of 33 milliseconds for defaultDecrementAmount will be
     ...
     * 
     * @param delta
     *            The number of milliseconds to decrement the timer
     */
     ...
     * 
     * @param duration
     *            The new duration for the MuleTimer
     */
     ...
     * 
     * @param amount
     *            A positive number of milliseconds to decrement the timer each
     *            time decrement() is called
     */
     ...
     * 
     * @param newDuration
     *            The new duration to be used for the timer
     */
	...
	double fractionRemaining = ((double) remainingTime)
		/ ((double) duration);
	...
	timer.schedule(new java.util.TimerTask() {
		...
		if (!tp.isFinished()) {
		    ...
		    g.drawString("Time is up!!!!", centerp.getWidth() / 3,
			    centerp.getHeight() / 2);
		    g.dispose();
		}
	    }
