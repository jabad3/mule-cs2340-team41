...
public class TownPanel extends JPanel{
     ...
     * 
     * @param comp1 First component being checked
     * @param comp2 Second component being checked
    ...
    /**
     * This method returns a new location of a JComponent that would place
     * the component completely in-bounds inside the Town.
     * 
     ...
     * 
     * @param shop The shop that the component is overlapping
     * @param component The component we wish to get a valid location for
     * @return Point representing a new, valid location for the component
     */
    private Point calcInBoundsLocation(JComponent shop,
         ...
         * 
         * For "corner-cases", the new location is chosen so that the
         * component will get moved the minimum amount to put it back in bounds.
         */
        if (shopBounds.contains(topLeft) && shopBounds.contains(topRight)) {  // overlaps bottom side
            newy = shopLocation.y + shopHeight;
        } else if (shopBounds.contains(topLeft) && shopBounds.contains(botLeft)) { // overlaps right side
            newx = shopLocation.x + shopWidth;
        } else if (shopBounds.contains(botLeft) && shopBounds.contains(botRight)) {  // overlaps top side
            newy = shopLocation.y - compHeight;
        } else if (shopBounds.contains(topRight) && shopBounds.contains(botRight)) {  // overlaps left side
            newx = shopLocation.x - compWidth;
        } else if (shopBounds.contains(topLeft)) {  // overlaps bottom-right corner
            ...
            if (dx < dy)
                ...
                newy = shopLocation.y + shopHeight;            
        } else if (shopBounds.contains(topRight)) {  // overlaps bottom-left corner
