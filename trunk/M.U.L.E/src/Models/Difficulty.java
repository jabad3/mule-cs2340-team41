...
/**
 * Difficulty enums for all possible difficulties in a game of MULE. Has getters
 * for player and store difficulty settings associated with each of the
 * different difficulty options.
 * 
 * @author Kevin
 */
public enum Difficulty {
    BEGINNER(8, 4, 0, 16, 16, 0, 25), STANDARD(4, 2, 0, 8, 8, 8, 14), TOURNAMENT(
	    4, 2, 0, 8, 8, 8, 14);
     ...
     * 
     * @param playerFood
     *            Starting food for Players
     * @param playerEnergy
     *            Starting energy for Players
     * @param playerOre
     *            Starting ore for Players
     * @param storeFood
     *            Starting food for Store
     * @param storeEnergy
     *            Starting energy for Store
     * @param storeOre
     *            Starting ore for Store
     * @param storeMuleCount
     *            Starting mule count for Store
     */
    private Difficulty(int playerFood, int playerEnergy, int playerOre,
	    int storeFood, int storeEnergy, int storeOre, int storeMuleCount) {
    ...
    /**
     * Returns name of the Difficulty with only the first letter capitalized for
     * more user-friendly display.
     */
    public String toString() {
	String allCaps = this.name();
	String firstLetter = allCaps.substring(0, 1); // capitalize first letter
	String remainingLetters = allCaps.substring(1).toLowerCase(); // all
								      // others
								      // are
								      // lowercase
