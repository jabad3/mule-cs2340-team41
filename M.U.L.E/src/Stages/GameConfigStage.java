...
import Models.Store;
import Views.BasicGameConfigView;
import Views.GameConfigView;
 ...
 * a) Difficulty b) Map Type c) Number of Players
 * 
 * The GameConfigStage class is responsible for: 1) Setting up its View with
 * valid choices 2) Listening for when the User signifies they are finished
 * interacting with the View (ex: the User presses "Next" or "Done") 3) Setting
 * GameModel's attributes according to User's input at the View
 * 
 * @author Max
 * 
 */
public class GameConfigStage extends Stage {
     ...
     * 
     * @param mainFrame
     *            Reference to the game's JFrame
     * @param model
     *            Reference to the game's GameModel
     */
    ...
    /**
     * Creates and configures an instance of the GameConfigView, then displays
     * it in the mainFrame to show to the user to start the stage.
     */
    public void start() {
    ...
    /**
     * Configures the model according to information in the View. The
     * information taken from the View is... 1) Difficulty 2) Map Type 3) Number
     * of players
     */
    ...
    /**
     * This class listens to the View for an ActionEvent signaling that the User
     * is finished interacting with the GameConfigView.
     * 
     * @author Max
     * 
     */
	...
	/**
	 * When the View is "finished", set up the model accordingly, then
	 * terminate this stage of the game by starting the nextStage.
	 * 
	 * @param e
	 *            ActionEvent fired by the GameConfigView
	 */
