...
/**
 * PlayerConfigStage is responsible for updating the Model using information in
 * the View during Player Configuration. The user is able to choose
 * characteristics for each Player, and these Player objects are created so that
 * they can later be added to the Model.
 * 
 * @author jabad3
 * 
 */
public class PlayerConfigStage extends Stage {
     ...
     * 
     * Colors must be disabled after they are picked so that no two Players have
     * the same Color.
     * 
     ...
     * 
     * @param mainFrame
     *            The primary container that will hold this Stage's view
     * @param model
     *            The GameModel that the Stage will manipulate
     */
    ...
    /**
     * Configures the view for the current player number, then displays the view
     * to the user.
     */
    ...
    /**
     * FinishedListener listens for when the user is finished configuring a
     * Player.
     * 
     * If there are still players left to configure, continue displaying
     * PlayerConfigViews to the user. Otherwise, there are no players left, so
     * the FinishedListener will take action to end the stage.
     * 
     * @author Max
     * 
     */
	...
	public void actionPerformed(ActionEvent e) {
	    ...
	    Player player = new Player(myView.getText(), myView.getRace(),
		    myView.getColor(), gameModel.getDifficulty());
