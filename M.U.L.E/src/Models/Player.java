 ...
 * player. This includes information such as the player's name, race, and color.
 * Players also maintain a list of plots that they own as well as the mule
 * object that they are currently holding, if they are holding one.
 * 
 * Player extends Trader, so Players contain an inventory which they change
 * after interactions with other Traders.
 * 
 * Player also implements Comparable<Player>, as Players need to be sorted to
 * determine turn order. Player ordering is based on the Player's current score.
 * Players with lower scores get to go first, so the compareTo() method is
 * implemented so that a sorted list of Players will have the Player with the
 * lowest score in the first position.
 * 
 * @author Max
 * 
 */
public class Player extends Trader implements Comparable<Player>, Serializable {
     ...
     *            The player's race
     * @param color
     *            The player's color for marking land plots
     * @param difficulty
     *            The game's set difficulty
     */
     ...
     * 
     * @param landPlot
     *            The landplot to add to the player's land plot list
     */
    ...
    /**
     * Returns a String containing information for the Player's instance data,
     * as well as information concerning its inventory. Intended to be printed
    ...
    /**
     * Returns String representation of inventory. Intended to be printed for
     * simple testing.
     * 
     * @return String representation of inventory
     */
    public String getMyInventoryAsString() {
	String s1 = "----------" + name + "'s Inventory--------------"
		+ inventory.toString();
	String s2 = "\nMy Score is: " + calculateScore();
	return s1 + s2;
    }
