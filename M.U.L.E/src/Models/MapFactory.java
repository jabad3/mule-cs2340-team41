 ...
 * map.
 * 
 * The two types of maps that the MapFactory can create are: 1) "Default" 2)
 * "Random"
 * 
 * The difference between the two maps is only in the arrangement of Plain and
 * Mountain LandPlots in the map.
 * 
 * If MapFactory is asked to build a map type that it cannot identify, it will
 * build a default map object.
 * 
 * @author Erica
 */
public class MapFactory {
    /**
     * Creates the map object depending on mapType. The String mapType should be
     * either "Default" or "Random". Any other String, including null, will
     * result in the default map being returned.
     * 
     * @param mapType
    ...
    /**
     * The method generates a 2D array of land plot types. The type at each
     * (row,col) coordinate corresponds to the type that the actual land plot at
     * that positions will have.
     * 
     * The arrangement of LandPlotTypes is based on the specification for the
     * default map.
     * 
    ...
    /**
     * This method generates a randomized 2D array of land plots similar to
     * generateDefaultMap().
     * 
     * The center row will still be the same as the default map (i.e. it will
     * have the river plots plus the town in the center)
     * 
     * The random map will have the same number of each LandPlotType that the
     * default map has.
     * 
	...
	for (int row = 0; row < typeMap.length; row++) {
		...
		// Leave middle column untouched
		if (col != 4) {
		    typeMap[row][col] = positionsToShuffle
			    .remove(positionsToShuffle.size() - 1);
		}
	    }
	}
	return typeMap;
