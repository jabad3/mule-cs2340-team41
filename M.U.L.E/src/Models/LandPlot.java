...
/**
 * The LandPlot class represents the model object for land tiles. LandPlots are
 * characterized by their type, their owner, and their mule. LandPlots do not
 * need to have mules installed, nor do they need to be owned. Owners and
 * installed mules can change over time, but their type remains the same for a
 * single game.
 * 
 * @author Max
 * 
...
public class LandPlot implements Serializable {
	...
	Map<LandPlotType, Map<Resource, Integer>> table = new EnumMap<>(
		LandPlotType.class);
     ...
     * 
     * @param landType
     *            The LandPlotType of the land plot
     */
     ...
     * 
     * @param player
     *            The new owner of the land plot
     */
    ...
    /**
     * A land plot is available to be purchased (or selected using a land grant)
     * as long as it has no owner and it is not the town.
     * 
     * @return True if the land plot can still be claimed by a Player, false
     *         otherwise
     */
    ...
    /**
     * Calculate the score value of a given land plot. If there is no mule on a
     * land plot, it's score value is 500 by default. If there is a mule on a
     * land plot, its score is determined solely by the installed mule.
     * 
     ...
     * 
     * @param player
     *            The player we are checking for ownership of the plot
     ...
     * 
     * @param mule
     *            The Mule to place on the LandPlot
     */
    ...
    /**
     * Gets the resource that this landplot will produce. Null if the land plot
     * cannot produce anything (i.e. land plot has no mule).
     * 
     * @return The Resource that this land plot will produce (null if the land
     *         plot cannot produce anything)
     */
    ...
    /**
     * Land plot produces its resource, increasing its owner's total. Production
     * occurs by: 1) Powering up the mule 2) Determine production results (# of
     * resource that was produced) 3) Adding resources to the owner's inventory
     * 
     * No production occurs if a) Plot is unowned b) Plot has no mule c) the
     * plot's mule cannot be powered (NOTE: energy mules do NOT require energy
     * units to function)
     */
    public void produce() {
	if (owner == null || mule == null || isTown()
		|| mule.getMuleType() == null) { // no mule, no production
     ...
     * 
     * Land plots with mules can only produce if the mule is an energy mule, or
     * if the mule is non-energy and has consumed an energy unit from its owner
     * 
     * @param resourceToProduce
     *            The resource that the land plot is configured to produce
	...
	lastAmountProduced = productionTable.get(landType).get(
		resourceToProduce);
