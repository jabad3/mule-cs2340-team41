...
/**
 * The DevelopmentView class allows the user to move their player around the map
 * and interact with the different shops in the town.
 * 
...
public class DevelopmentView extends JPanel {
     ...
     * @param townPanel
     *            The town to display to the user
     * @param playerPawn
     *            The first pawn to display to the user
     * @param muleTimerPanel
     *            The timer bar to display to the user
     */
    public DevelopmentView(MapPanel mapPanel, TownPanel townPanel,
	    PlayerPawn playerPawn, MuleTimerPanel muleTimerPanel) {
    ...
    /**
     * Updates the view to display player info for the given player. This
     * information includes: 1) Name, and 2) Inventory (Money, Food, Energy,
     * Ore) information
     * 
     * @param player
     *            The player to display the info for
     */
	...
	getCurrentPawn().setLocation(mapPanel.getX() + mapPanel.getWidth() / 2,
    ...
    /**
     * This method ends a player's turn by preventer further interaction with
     * the View.
     */
     ...
     * 
     * This method should be called on a frequently enough so that the animation
     * appears smooth.
     */
	...
	setFocusable(true); // if not called, then muleTimerPanel.repaint()
			    // messes up focus
    }

    /**
     * Updates previousActionKeyState and currentActionKeyState according to
     * what the user has pressed at this point in time.
     */
	...
	} else if (pawnPos.y + pawnSize.height > panelOrigin.y
		+ panelSize.height) {
    ...
    /**
     * In the map, take action if the current pawn... 1) Collides with the town,
     * or 2) Collides with the map border
     */
    ...
    /**
     * Checks whether or not the user has just hit the action key. A hit key is
     * defined as a key that is pressed now that was previously unpressed.
     * 
    ...
    /**
     * In the map, take action if the current pawn.. 1) Collides with the town
     * border, 2) Collides with an entrance to a shop, or 3) Collides with a
     * shop border that is not an entrance
     */
	...
	if (!townPanel.insideTown(getCurrentPawn())) {
		...
		} else if (getCurrentPawn().getLocation().x > townPanel
			.getWidth() - 60) {
	...
	if (townPanel.overlapsPubEntrance(getCurrentPawn())
		&& getCurrentPawn().actionKey) {
	    sendEnteredPubNotifications();
	}

	if (townPanel.overlapsStoreEntrance(getCurrentPawn())
		&& actionKeyWasHit()) {
     ...
     * 
     * @param enteredPlot
     *            The LandPlot that the user interacted with
     */
     ...
     * 
     * @param newImage
     *            The new image to be displayed in the current PlayerPawn
     */
    public void setCurrentPawnImageIcon(ImageIcon newImage) {
	getCurrentPawn().setIcon(newImage);
	getCurrentPawn()
		.setSize(
			new Dimension(newImage.getIconWidth(), newImage
				.getIconHeight()));
    }
     ...
     * 
     * The ShopEntryListener will be notified when the user interacts with one
     * of the shops during development.
     * 
     * @param sel
     *            The ShopEntryListener to add to the view
     */
    ...
    /**
     * Displays a simple dialog box to the user containing a message. This
     * message has no effect other than relaying a message to the user.
     * 
     * @param message
    ...
    // final MapPanel mapPanel = new MapPanel(MapFactory.buildMap("Default"),
    // null);
    ...
    //
    // final DevelopmentView dv = new DevelopmentView(mapPanel, townPanel, pawn,
    // muleTimerPanel);
    //
